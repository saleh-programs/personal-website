...
function Bar({name, subname, subname2, contentID, date, link}){
  ...
  const [keepStored, setKeepStored] = useState(false);
  const keepStoredTimerRef = useRef(false);
  ...
  function toggleBar(){
    clearTimeout(keepStoredTimerRef.current);
    if (isExpanded){
      setIsExpanded(false);
      keepStoredTimerRef.current = setTimeout(() => {
        setKeepStored(false);
      }, 5000);
    }else{
      setIsExpanded(true);
      ...
      <div className={styles['dropdown-ref']}>
        ...
        <div className={styles.bar} onClick={toggleBar}>
          ...
          <div className={styles.contentContainer} style={{maxHeight: contentMaxHeight}} ref={elem}>
            {keepStored && contentDict[contentID]}
          </div>
        </div>
      </div>
  );
