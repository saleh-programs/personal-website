...
function Navbar({refs}){
  const [homeRef, educationRef, projectsRef, experienceRef, scrollableRef] = refs;
...
function updateActiveSection() {
  ...
  const sections = [homeRef, educationRef, projectsRef, experienceRef]
...
useEffect(()=>{console.log(activeId)}, [activeId])
  ...
  return(
      <div className={styles.navbar}>
        ...
        <button onClick={()=>handleClick(experienceRef)} className={`${styles.link} ${activeId === "experience" ? styles.highlight : ""}`}>Experience/Skills</button>
      </div>
  );
}
